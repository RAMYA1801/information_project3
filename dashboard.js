...
function drawPie(selector, data) {
  ...
  g.selectAll("path")
    ...
    .on("click", function(event, d) {
      if (selectedRegion === d.data[0]) {
        selectedRegion = null;
        updateDashboard(fullData);
      } else {
        selectedRegion = d.data[0];
        const filtered = fullData.filter(row => row.Origin === selectedRegion);
        updateDashboard(filtered);
      }
    });

  // Add Region Labels inside Pie
  g.selectAll("text")
    .data(pie(pieData))
    .enter()
    .append("text")
  ...
  if (selectedRegion) {
    g.selectAll("path")
      .attr("opacity", d => d.data[0] === selectedRegion ? 1 : 0.4);
  }
}
